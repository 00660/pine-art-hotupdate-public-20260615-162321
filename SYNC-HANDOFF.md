...
全量 ready 构建 run `26696262658` 已取消。取消原因：`beryllium` 老内核在 32-bit vDSO 链接阶段使用宿主 `/usr/bin/ld`，真实错误为 `unrecognised emulation mode: armelf_linux_eabi`。已在 `scripts/build-lineage-recipe.sh` 的 kernel `MAKE_ARGS` 中加入 `CROSS_COMPILE_COMPAT=arm-linux-gnueabi-`，让老 arm64 kernel 的 compat/vdso32 链接走 ARM32 binutils。
